...
{
   // cout << "simbad::gui::Cell_Map::setInitConfiguration(Points_of_model & Model_P)" << endl;
...
//-------------------------------------------------------
    ...
    for ( int k=0; k < Model_P.Number_of_types; k++)
    {
        for( int i = 0;i < Model_P.Vector_of_types[k].Number_of_points_in_SpacePointArray; i++){
            ...
            int addtional_variable=(int) floorf(10*Model_P.Vector_of_types[k].SpacePointArray[i].Xcoordinate +
                matrix_scaling_factor*5) +
                matrix_scaling_factor*10*floorf(10*Model_P.Vector_of_types[k].SpacePointArray[i].Ycoordinate +
                matrix_scaling_factor*5);
    ...
    for ( int i = 0 ; i < 10; ++i)
        for ( int j = 0 ; j < 10; ++j)
        {
              ...
              for (int k=0; k<Model_P.Number_of_types; k++)
              {
                  int addtional_variable=(int) floorf(matrix_scaling_factor * 5 + j  - 5 +
                  ...
                  if (Array_of_cells[addtional_variable][k].size() >
                      ...
                      int size_last = Types_Array_numbers_of_sizes_of_cells[k].size();
                          for(int m = size_last;
                              m < Array_of_cells[addtional_variable][k].size();
                              m ++) Types_Array_numbers_of_sizes_of_cells[k].push_back(0);

                          Types_Array_numbers_of_sizes_of_cells[k].push_back(1);
                  } else {

                      //нужно резервировать первый элемент (нулевой в массиве) с 0, иначе вылет (это сделано в начале этой функции)
                      Types_Array_numbers_of_sizes_of_cells[k]
                              [Array_of_cells[addtional_variable][k].size()]=
                          Types_Array_numbers_of_sizes_of_cells[k]
                              [Array_of_cells[addtional_variable][k].size()] + 1;
                  };
