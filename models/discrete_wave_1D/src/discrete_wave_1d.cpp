...
discrete_wave_1d::discrete_wave_1d(double alpha, double intensity_cap,
                                   ...
                                   std::size_t seed)
    : impl(std::make_unique<discrete_wave_1d_impl>(alpha, intensity_cap, length,
                                                   spacing, seed))
{
  impl->initial_event();
}

discrete_wave_1d::~discrete_wave_1d() {}

void discrete_wave_1d::generate_events(event_visitor visitor,
                                       size_t nevents) const
{
  CORE_NAMESPACE::simple_event_view<discrete_wave_1d_impl::Event> event_view;
  for (size_t i = 0; i < nevents; ++i)
  {
    discrete_wave_1d_impl::Event event = impl->next_event();
    event_view = event;
    visitor(event_view);
  }
}

std::size_t discrete_wave_1d::configuration_size() const
{
...
void discrete_wave_1d::visit_configuration(particle_visitor v) const
{
