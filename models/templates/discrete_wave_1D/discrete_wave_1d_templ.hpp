...
public:
  ...
  discrete_wave_1d_impl(double alpha, double intensity_cap, std::size_t length,
                   double x0=1, double spacing = 1.0, std::size_t seed = 0)
      : time(0), intensity_cap(intensity_cap), grid_spacing(spacing),
        random_engine(seed), birth_power_law(1.0, alpha), grid_length(length),
        grid_count(grid_length, 0), birth_density(grid_length, 0.0),
  ...
  Event next_event() {
    ...
    std::size_t birth_position = static_cast<Derived*>(this)->sample_birth_pos(random_engine);
    ++grid_count[birth_position];

    Event e(time, simbad::core::EVENT_KIND::BIRTH);
    e.set_coordinate(0, static_cast<Derived*>(this)->distance_from_origin(birth_position));
...
protected:
  void update_birth_uncapped_density(const Event &e) {
    ...
    for (std::vector<double>::iterator it = beg; it < end; ++it) {
      ...
      const double xpos = static_cast<Derived*>(this)->distance_from_origin(xind);
