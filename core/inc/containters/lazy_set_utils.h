...
typedef boost::intrusive::avl_set_base_hook<boost::intrusive::optimize_size<true> > tree_hook;
...
struct ChunkContains{
    bool operator()(LazySetNode const &node, D const &data) const{
        return static_cast<const void*>(&node) < static_cast<const void*>(&data - LazySetNode::max_occupancy() );
    }
    bool operator()(D const &data, LazySetNode const &node ) const{
        return static_cast<const void*>(&data + LazySetNode::length()) < static_cast<const void*>(&node);
    }
};
...
struct DifferentFromNodePred{
    ...
    bool operator()(LazySetNode const &node) const{
        return &node != pNode;
    }
    LazySetNode *pNode;
};
