...
template <class LoadFactorType> struct auto_rehash
{
  explicit auto_rehash(LoadFactorType max_load_factor = 1)
      : m_max_load_factor(max_load_factor)
  {
  }
  template <class OrderedBoard> void operator()(OrderedBoard &board) const
  {
    using size_type = typename OrderedBoard::size_type;
    size_type sz = board.size();
    size_type nbuckets = board.bucket_count();
