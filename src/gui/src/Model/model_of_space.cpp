...
QStringList simbad::gui::Model_of_space::get_results_of_simulation_for_file()
{
  ...
  Result_Q_String_List.push_back(
      "Number of events: " +
      QString::number(this->Vector_of_result_event.size()) + "\n");
  ...
  for(int i = 0; i < this->Vector_of_result_event.size(); i++)
  {
    ...
    //"Time of Event: "
    Result_Q_String_List.push_back(
        QString::number((double)this->Vector_of_result_event[i]
                            .get_global_time_of_result_event()) +
        "\n");

    // Result_Q_String_List.push_back("Dead points of configuration after
    // event:\n"); "Dead point type: " +
    Result_Q_String_List.push_back(
        QString::number(this->Vector_of_result_event[i]
    ...
    // Result_Q_String_List.push_back("New points of configuration after
    // event:\n"); "New point type: "
    Result_Q_String_List.push_back(
        QString::number(this->Vector_of_result_event[i]
...
QStringList simbad::gui::Model_of_space::get_configuration_setting_for_file()
{
  ...
  {
    int red =
        this->ModelPoints.Vector_of_types[i].InitialConfigurationColor.red();
    int green =
        this->ModelPoints.Vector_of_types[i].InitialConfigurationColor.green();
    int blue =
        this->ModelPoints.Vector_of_types[i].InitialConfigurationColor.blue();
    ...
    Conf_Q_String_List.push_back(
        "Number of points in type " + QString::number(i + 1) + ": " +
        QString::number(this->ModelPoints.Vector_of_types[i]
                            .Number_of_points_in_SpacePointArray) +
        "\n");

    for(int j = 0; j < this->ModelPoints.Vector_of_types[i]
                           .Number_of_points_in_SpacePointArray;
        j++)
    {
      Conf_Q_String_List.push_back(
...
QStringList simbad::gui::Model_of_space::get_model_settings_for_file()
{
  ...
  for(int i = 0; i < this->List_of_events_of_model.size(); ++i)
  {
    ...
    // for (int j = 0; j< this->List_of_events_of_model[i].Code_of_event.size();
    // j++){

    //    for (int k = 0; k <
    //    this->List_of_events_of_model[i].Code_of_event[j].size(); k++){
    //         Model_Q_String_List.push_back(QString::number(this->List_of_events_of_model[i].Code_of_event[j][k]+1));
    //         if (k !=
    //         (this->List_of_events_of_model[i].Code_of_event[j].size()-1))
    //         Model_Q_String_List.push_back("->");
    //    }
    //    Model_Q_String_List.push_back("\n");
    ...
    for(int j = 0;
        j < this->List_of_events_of_model[i]
                .Table_of_component_rates_for_event_of_model.Table.size();
        j++)
    {
      Model_Q_String_List.push_back(
          "influent particle type: " +
          QString::number(
              this->List_of_events_of_model[i]
                  .Table_of_component_rates_for_event_of_model.Table[j]
                  .get_influent_particle_type()) +
          "\n");
      ...
      if(Func == exp_minus)
        Str_Func = "exp_minus";
      if(Func == zero_next_to_null)
        Str_Func = "zero_next_to_null";
      Model_Q_String_List.push_back("Function: " + Str_Func + "\n");

      Model_Q_String_List.push_back(
          "Multiplication by constant of function: " +
          QString::number(
              this->List_of_events_of_model[i]
                  .Table_of_component_rates_for_event_of_model.Table[j]
      ...
      if(Approach == exp_minus_sum)
        Str_Approach = "exp_minus_sum";
      if(Approach == exp_plus_sum)
        Str_Approach = "exp_plus_sum";
      if(Approach == product)
    ...
    Model_Q_String_List.push_back(
        "Type of T student distribution: " +
...
bool simbad::gui::Model_of_space::set_configiguration_setting_from_file(
    QStringList String_list)
{
     ...
     this->get_number_of_types() == Number_of_typ)
  {
    ...
    for(int i = 0; i < this->get_number_of_types(); i++)
    {
      ...
      this->ModelPoints.Vector_of_types[i].InitialConfigurationColor.setRed(
          String_list[current_line_number].toInt());
      String_list[++current_line_number].remove("Green: ");
      this->ModelPoints.Vector_of_types[i].InitialConfigurationColor.setGreen(
          String_list[current_line_number].toInt());
      String_list[++current_line_number].remove("Blue: ");
      this->ModelPoints.Vector_of_types[i].InitialConfigurationColor.setBlue(
          String_list[current_line_number].toInt());

      ++current_line_number;
      String_list[++current_line_number].remove("Red: ");
      this->ModelPoints.Vector_of_types[i].DynamicSimulationColor.setRed(
          String_list[current_line_number].toInt());
      String_list[++current_line_number].remove("Green: ");
      this->ModelPoints.Vector_of_types[i].DynamicSimulationColor.setGreen(
          String_list[current_line_number].toInt());
      String_list[++current_line_number].remove("Blue: ");
      this->ModelPoints.Vector_of_types[i].DynamicSimulationColor.setBlue(
          String_list[current_line_number].toInt());

      String_list[++current_line_number].remove("Number of points in type " +
                                                QString::number(i + 1) + ": ");
          ...
          j++)
      {
        ...
        this->ModelPoints.Vector_of_types[i].SpacePointArray[j].BirthTime =
            String_list[current_line_number].toDouble();

        String_list[++current_line_number].remove("number of point in cell: ");
        this->ModelPoints.Vector_of_types[i]
            .SpacePointArray[j]
            .number_of_point_in_cell = String_list[current_line_number].toInt();

        String_list[++current_line_number].remove("X coordinate: ");
        this->ModelPoints.Vector_of_types[i].SpacePointArray[j].Xcoordinate =
            String_list[current_line_number].toFloat();

        String_list[++current_line_number].remove("Y coordinate: ");
        this->ModelPoints.Vector_of_types[i].SpacePointArray[j].Ycoordinate =
            String_list[current_line_number].toFloat();
      };
    };
...
bool simbad::gui::Model_of_space::set_model_setting_for_model_from_file(
    QStringList String_list)
{
  ...
  for(int i = 0; i < Nuberm_of_events; i++)
  {
    ...
    Current_event.set_number_of_types_for_event_of_model(
        this->get_number_of_types());

    Current_event.set_code_of_event(Current_event.get_name_of_event());

    Current_event.Table_of_component_rates_for_event_of_model.Number_of_types =
        this->get_number_of_types();
    ...
    for(int i = 0; i <= this->get_number_of_types(); i++)
    {
      ...
      Function_of_com_rate.set_influent_particle_type(
          String_list[current_line_number].toInt());
      ...
      if(Str_Func == "null")
        Func = null;
      if(Str_Func == "triangle")
        Func = triangle;
      if(Str_Func == "modul")
        Func = modul;
      if(Str_Func == "one")
        Func = one;
      if(Str_Func == "exp_minus")
        Func = exp_minus;
      if(Str_Func == "zero_next_to_null")
        Func = zero_next_to_null;
      Function_of_com_rate.set_Function(Func);

      String_list[++current_line_number].remove(
          "Multiplication by constant of function: ");
      ...
      Function_of_com_rate.set_Restrictions_from_string(
          String_list[current_line_number]);
      ...
      if(Str_Approach == "sum")
        Appro = sum;
      if(Str_Approach == "exp_minus_sum")
        Appro = exp_minus_sum;
      if(Str_Approach == "exp_plus_sum")
        Appro = exp_plus_sum;
      if(Str_Approach == "product")
        Appro = product;
      ...
      Function_of_com_rate.set_Constant_rate_plus(
          String_list[current_line_number].toFloat());
      Current_event.Table_of_component_rates_for_event_of_model.Table.push_back(
          Function_of_com_rate);
    };

    String_list[++current_line_number].remove("Type of new particle: ");
    Current_event.Distribution_of_particle_for_event.set_Type_of_particle(
        String_list[current_line_number].toInt());
...
void simbad::gui::Model_of_space::push_back_new_event(
    const Event_of_model &New_event)
{
  this->List_of_events_of_model.push_back(New_event);
}

QVector<simbad::gui::Event_of_model>
simbad::gui::Model_of_space::get_vector_of_events()
{
...
bool simbad::gui::Model_of_space::timer()
{
  ...
  for(int i = 0; i < this->List_of_events_of_model.size(); i++)
    for(int j = 0; j < this->get_number_of_types() + 1; j++)
    {
      ...
      {
        int Type_of_event_parent_point =
            this->List_of_events_of_model[i].Code_of_event[0][0];
        // /////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // comulative rate depends on general Nubmer of parent points and
        // max(Dzeta rate)_rate of probable neighbors  // with corresponding
        // type of influence //
  ...
  //      Dzeta_a_minus= 4.0 * (Array_numbers_of_sizes_of_cells.size() - 1) *
  //      f_a_minus(0)/5.0;
}
...
void simbad::gui::Model_of_space::initiate_before_simulation()
{
  ...
  // reserving memory
  Vector_time_before_next_event_of_type.resize(
      this->List_of_events_of_model.size());

  for(int i = 0; i < this->List_of_events_of_model.size(); i++)
  {
    Vector_time_before_next_event_of_type[i].resize(
        this->get_number_of_types() + 1);
  };

  // comulative rates of type events (type events)
  // reserving memory

  Vector_comulative_rate_for_type_of_event.resize(
      this->List_of_events_of_model.size());

  for(int i = 0; i < this->List_of_events_of_model.size(); i++)
  {
    Vector_comulative_rate_for_type_of_event[i].resize(
        this->get_number_of_types() + 1);
  };
  ...
  for(int i = 0; i < this->List_of_events_of_model.size(); i++)
  {
    for(int j = 1;
        ...
        j++)
    {
      if(max_range_of_functions <
         this->List_of_events_of_model[i]
             .Table_of_component_rates_for_event_of_model.Table[j]
             .get_Range_of_function())
        max_range_of_functions =
            this->List_of_events_of_model[i]
                .Table_of_component_rates_for_event_of_model.Table[j]
                .get_Range_of_function();
    };
    ...
    for(int i = 0; i < this->List_of_events_of_model.size(); i++)
    {
          ...
          j++)
      {
        ...
        if(max_range_of_functions > old_range_of_function)
        {
          this->List_of_events_of_model[i]
              .Table_of_component_rates_for_event_of_model.Table[j]
              .set_Range_of_the_function(old_range_of_function /
                                         max_range_of_functions * 0.05);
        }
        else
        {
          this->List_of_events_of_model[i]
              .Table_of_component_rates_for_event_of_model.Table[j]
              .set_Range_of_the_function(0.05);
        };
      };
    };
  };
    ...
    for(int j = 1; j < this->get_number_of_types() + 1; j++)
    {
      ...
      Vector_max_of_rate_functions[i][j] =
          this->List_of_events_of_model[i]
              .Table_of_component_rates_for_event_of_model.Table[j]
              .get_max_of_Function();
      // cout<<"Vector_max_of_rate_functions["<<i<<"]["<<j<<"]"<<Vector_max_of_rate_functions[i][j]
      // <<endl;
      ...
      // <<endl;  cout<<i<<",*"<<j<<","<<Vector_of_Dzeta_type_event[i][j]<<endl;
    };
  };
}

void simbad::gui::Model_of_space::Event_execution(
    int Number_of_event, int Number_function_for_component_rate)
{
  // cout<<"yes"<<endl;
  ...
  if(Number_of_involved_points_in_event == 1)
  {
    ...
    if(this->List_of_events_of_model[Number_of_event].Code_of_event[0][1] == -1)
    {
      ...
      // cout<<"Number_of_event="<<Number_of_event<<"
      // Number_function_for_component_rate="<<Number_function_for_component_rate<<endl;
      death_of_particle(Number_of_event, Number_function_for_component_rate);
    }
  };
}

void simbad::gui::Model_of_space::birth_of_particle(
    int EventNumber, int Numberfunctionforcomponentrate)
{
  unsigned seed;
  seed = std::chrono::system_clock::now().time_since_epoch().count();
  ...
  int number_of_type_of_child =
      this->List_of_events_of_model[EventNumber].Code_of_event[1][1];
  ...
  uniform_int_distribution<int> distribution_1(0, Number_of_parent_type_points -
                                                      1);

  int number_of_parent = distribution_1(mtUniDistribution);

  float rate =
      rate_linear_on_configuration(EventNumber, Numberfunctionforcomponentrate,
                                   number_of_type_of_parent, number_of_parent);

  bernoulli_distribution distribution_2(
      rate * 0.999 /
  ...
  if(bernulli_D)
  {
    ...
    if(this->List_of_events_of_model[EventNumber]
           .Distribution_of_particle_for_event
           .get_Type_of_T_student_distribution() == 0)
    {
      normal_distribution<float> Normal_distribution(
          0.0, this->List_of_events_of_model[EventNumber]
                   .Distribution_of_particle_for_event.get_scale_paramiter());

      Xcoordinate_of_new_particle =
          this->ModelPoints.Vector_of_types[number_of_type_of_parent]
              .SpacePointArray[number_of_parent]
              .Xcoordinate +
    ...
    else
    {
      uniform_real_distribution<float>
          uniform_distribution_angle_for_t_student_distribution(0.0, 2.0 * PI);
      student_t_distribution<float> Student_T_Distribution(
          this->List_of_events_of_model[EventNumber]
              .Distribution_of_particle_for_event
              .get_Type_of_T_student_distribution());

      float Student_t_Distibution =
          this->List_of_events_of_model[EventNumber]
              .Distribution_of_particle_for_event.get_scale_paramiter() *
          fabs(Student_T_Distribution(mtUniDistribution));
      float angle = uniform_distribution_angle_for_t_student_distribution(
          mtUniDistribution);
    ...
    one_birth(number_of_type_of_child, Xcoordinate_of_new_particle,
              Ycoordinate_of_new_particle);
    ...
    if((Saving_step > 0) && (Number_of_occurings % Saving_step == 0))
      add_new_result_of_dynamics_one_beath_and_one_death(
          Global_clock, Xcoordinate_of_new_particle,
          Ycoordinate_of_new_particle, number_of_type_of_child, 0, 0, -1);
  };
}

void simbad::gui::Model_of_space::birth_of_particle_from_environment(
    int EventNumber)
{
  unsigned seed;
  seed = std::chrono::system_clock::now().time_since_epoch().count();
  ...
  float range_of_function =
      this->List_of_events_of_model[EventNumber]
          .Table_of_component_rates_for_event_of_model.Table[0]
          .get_Range_of_function();
  uniform_real_distribution<double> uni_distribution(-range_of_function,
                                                     range_of_function);
  ...
  if((Saving_step > 0) && (Number_of_occurings % Saving_step == 0))
    add_new_result_of_dynamics_one_beath_and_one_death(
        Global_clock, Xcoordinate_of_new_particle, Ycoordinate_of_new_particle,
        number_of_type_of_child, 0, 0, -1);
}
                                            ...
                                            float Ycoordinate_of_new_particle)
{
  ...
  this->ModelPoints.Vector_of_types[number_of_type_of_child]
      .SpacePointArray[Number_of_child_type_points - 1]
      .Xcoordinate = Xcoordinate_of_new_particle;
  this->ModelPoints.Vector_of_types[number_of_type_of_child]
      .SpacePointArray[Number_of_child_type_points - 1]
      .Ycoordinate = Ycoordinate_of_new_particle;
  // cout<<"q_1"<<endl;
  ...
  // если ячейка с новой точкой стала содержать максимальное кол-во элементов
  // среди всех ячеек, то

  if(this->CellMap.Array_of_cells[addtional_variable][number_of_type_of_child]
         .size() >
     this->CellMap
             .Types_Array_numbers_of_sizes_of_cells[number_of_type_of_child]
             .size() -
         1)
  {
    //уменьшаем кол-во ячеек с предыдущим максимумом (он на ед. меньше чем
    //новый)
    ...
    this->CellMap.Types_Array_numbers_of_sizes_of_cells
        [number_of_type_of_child]
        [this->CellMap
             .Array_of_cells[addtional_variable][number_of_type_of_child]
             .size() -
         1] =
        this->CellMap.Types_Array_numbers_of_sizes_of_cells
            [number_of_type_of_child]
    ...
    // cout<<"4"<<endl;
    //           Array_numbers_of_sizes_of_cells[Array_numbers_of_sizes_of_cells.size()
    //           - 1]=
    //              Array_numbers_of_sizes_of_cells[Array_numbers_of_sizes_of_cells.size()
    //              - 1] - 1;
    //увеличиваем максимум  (тоесть кол-во ячеек с новым максимумом уже стало
    //ровняться 1)
  ...
  else
  {
    ...
    //      Array_numbers_of_sizes_of_cells[Array_of_cells_a_minus[addtional_variable].size()
    //      - 1] - 1;
    // увеличиваем кол-во ячеек на +1 с размером
    // Array_of_cells_a_minus[addtional_variable].size() (так-как после
    // повяления нового єлемента
    ...
    int EventNumber, int Numberfunctionforcomponentrate)
{
  ...
  {
    float rate = rate_linear_on_configuration(
        EventNumber, Numberfunctionforcomponentrate,
        number_of_type_of_jump_particle, number_of_jump_man);
    bernoulli_distribution distribution_2(
        rate * 0.999 /
        Vector_of_Dzeta_type_event[EventNumber]
                                  [Numberfunctionforcomponentrate]);
    ...
    if(bernulli_D)
    {
      ...
      Xcoordinate_of_jump_particle =
          this->ModelPoints.Vector_of_types[number_of_type_of_jump_particle]
              .SpacePointArray[number_of_jump_man]
              .Xcoordinate;

      Ycoordinate_of_jump_particle =
          this->ModelPoints.Vector_of_types[number_of_type_of_jump_particle]
              .SpacePointArray[number_of_jump_man]
              .Ycoordinate;
             ...
             .get_Type_of_T_student_distribution() == 0)
      {
        ...
        New_Xcoordinate_of_jump_particle =
            Xcoordinate_of_jump_particle +
            Normal_distribution(mtUniDistribution);

        New_Ycoordinate_of_jump_particle =
            Ycoordinate_of_jump_particle +
            Normal_distribution(mtUniDistribution);
      }
      else
      {
        ...
        uniform_real_distribution<float>
            uniform_distribution_angle_for_t_student_distribution(0.0,
                                                                  2.0 * PI);
        ...
        New_Xcoordinate_of_jump_particle =
            Xcoordinate_of_jump_particle + Student_t_Distibution * cos(angle);

        New_Ycoordinate_of_jump_particle =
            Ycoordinate_of_jump_particle + Student_t_Distibution * sin(angle);
      };

      one_death(number_of_type_of_jump_particle, number_of_jump_man);
      one_birth(
          number_of_type_of_jump_particle, New_Xcoordinate_of_jump_particle,
          New_Ycoordinate_of_jump_particle); // number_of_type_of_jump_particle
  ...
  else
  {
           ...
           .get_Type_of_T_student_distribution() == 0)
    {
      ...
      New_Xcoordinate_of_jump_particle =
          Xcoordinate_of_jump_particle + Normal_distribution(mtUniDistribution);

      New_Ycoordinate_of_jump_particle =
          Ycoordinate_of_jump_particle + Normal_distribution(mtUniDistribution);
    }
    else
    {
      student_t_distribution<float> Student_T_Distribution(
...
void simbad::gui::Model_of_space::mutation_of_particle(
    int EventNumber, int Numberfunctionforcomponentrate)
{
  unsigned seed;
  seed = std::chrono::system_clock::now().time_since_epoch().count();
  ...
  int number_of_new_type_of_mutated_particle =
      this->List_of_events_of_model[EventNumber].Code_of_event[0][1];
  //  cout<<
  //  "number_of_type_of_death_particle="<<number_of_type_of_death_particle<<endl;
  int Number_of_points =
  ...
  if(Numberfunctionforcomponentrate != 0)
  {
    float rate = rate_linear_on_configuration(
        EventNumber, Numberfunctionforcomponentrate,
        number_of_old_type_of_mutated_particle, number_of_mutated_man);
    // cout<< "rate="<<rate<<endl;
    // cout<<"Vector_of_Dzeta_type_event[EventNumber][Numberfunctionforcomponentrate]"<<
    //       Vector_of_Dzeta_type_event[EventNumber][Numberfunctionforcomponentrate];
    // bernoulli_distribution
    // distribution_2(rate*0.999/Vector_comulative_rate_for_type_of_event[EventNumber][Numberfunctionforcomponentrate]);
    bernoulli_distribution distribution_2(
        rate * 0.999 /
        Vector_of_Dzeta_type_event[EventNumber]
                                  [Numberfunctionforcomponentrate]);
    ...
    if(bernulli_D)
    {
      float Xcoordinate_of_mutated_particle =
          this->ModelPoints
              .Vector_of_types[number_of_old_type_of_mutated_particle]
              .SpacePointArray[number_of_mutated_man]
              .Xcoordinate;

      float Ycoordinate_of_mutated_particle =
          this->ModelPoints
              .Vector_of_types[number_of_old_type_of_mutated_particle]
              .SpacePointArray[number_of_mutated_man]
              .Ycoordinate;
      ...
      // uniform_real_distribution<float>
      // uniform_distribution_angle_for_t_student_distribution(0.0,2.0*3.14);
      // student_t_distribution<float> Student_T_Distribution(0);
      // normal_distribution<float> Normal_distribution(0.0,0.1);

      // float angle_for_student_t_distribution =
      // uniform_distribution_angle_for_t_student_distribution(mtUniIntDistribution);
      ...
      // float New_Ycoordinate_of_jump_particle = Ycoordinate_of_jump_particle +
      // Normal_distribution(mtUniRealDistribution)*
      //        sin(angle_for_student_t_distribution);

      one_death(number_of_old_type_of_mutated_particle, number_of_mutated_man);
      one_birth(
          number_of_new_type_of_mutated_particle,
          Xcoordinate_of_mutated_particle,
          Ycoordinate_of_mutated_particle); // number_of_type_of_jump_particle

      //            Global_clock = Global_clock + Min_time;
      // if
      // (this->ModelPoints.Vector_of_types[number_of_new_type_of_mutated_particle].SpacePointArray.size()
      // != 0)
      //{
  ...
  else
  {
    ...
    one_death(number_of_old_type_of_mutated_particle, number_of_mutated_man);
    one_birth(
        number_of_new_type_of_mutated_particle, Xcoordinate_of_mutated_particle,
        Ycoordinate_of_mutated_particle); // number_of_type_of_jump_particle
...
void simbad::gui::Model_of_space::death_of_particle(
    int EventNumber, int Numberfunctionforcomponentrate)
{
  unsigned seed;
  seed = std::chrono::system_clock::now().time_since_epoch().count();
  ...
  if(Numberfunctionforcomponentrate != 0)
  {
    float rate = rate_linear_on_configuration(
        EventNumber, Numberfunctionforcomponentrate,
        number_of_type_of_death_particle, number_of_dead_man);
    // cout<< "rate="<<rate<<endl;
    // cout<<"Vector_of_Dzeta_type_event[EventNumber][Numberfunctionforcomponentrate]"<<
    //       Vector_of_Dzeta_type_event[EventNumber][Numberfunctionforcomponentrate];
    // bernoulli_distribution
...
void simbad::gui::Model_of_space::one_death(
    int number_of_type_of_death_particle, int number_of_dead_man)
{
  int Number_of_points =
      this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
          .Number_of_points_in_SpacePointArray;
  // информация о будущей мертвой точки =  координаты + номер который имеет
  // точка в ячейке
  float X_coord_of_dad_man =
      this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
          .SpacePointArray[number_of_dead_man]
          .Xcoordinate;
  float Y_coord_of_dad_man =
      this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
          .SpacePointArray[number_of_dead_man]
          .Ycoordinate;

  int Cell_num_of_dad_man =
      this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
          .SpacePointArray[number_of_dead_man]
          .number_of_point_in_cell;
  //
  // addtional_variable - номер ячейки в которую входит будущая мертвая точка с
  // номером number_of_dead_man
  int addtional_variable =
      (int)floorf(10 * X_coord_of_dad_man + matrix_scaling_factor * 5) +
      10 * matrix_scaling_factor *
          floorf(10 * Y_coord_of_dad_man + matrix_scaling_factor * 5);
  ...
  if(Number_of_points - 1 != number_of_dead_man)
  {
    ...
    //						cerr << "size of Array_of_cells_a_minus[addtional_variable]
    //"<< Array_of_cells_a_minus[addtional_variable].size() << '\n';
    //			};
    ...
    if((this->CellMap.Types_Array_numbers_of_sizes_of_cells
            [number_of_type_of_death_particle]
            [this->CellMap
                 .Array_of_cells[addtional_variable]
                                [number_of_type_of_death_particle]
    ...
    // и увеличивам кол-во ячеек с размером
    // Array_of_cells_a_minus[addtional_variable].size()-1 на 1;
    ...
    this->CellMap.Types_Array_numbers_of_sizes_of_cells
        ...
        [this->CellMap
             .Array_of_cells[addtional_variable]
                            [number_of_type_of_death_particle]
             .size() -
         1] = this->CellMap.Types_Array_numbers_of_sizes_of_cells
                  [number_of_type_of_death_particle]
                  [this->CellMap
                       .Array_of_cells[addtional_variable]
                                      [number_of_type_of_death_particle]
    ...
    //В этой переменной мы сохраняем номер элемента главного массива точек на
    //который указывает крайний элемент ячейки addtional_variable
    // то-есть ячейки в которой находиться будущий мертвый элемент
    int Number_of_point_in_array_of_points =
        CellMap.Array_of_cells
            [addtional_variable][number_of_type_of_death_particle]
    ...
    //тут мы меняем  номер number_of_point_in_cell элемента главного массива
    //точек на который указывает крайний элемент ячейки на номер точки в ячейки
    //будуще мертвой точки

    this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
        .SpacePointArray[Number_of_point_in_array_of_points]
        .number_of_point_in_cell = Number_of_point_of_dead_man_in_cell;
    //        Array_of_points[Number_of_point_in_array_of_points].number_of_point_in_cell
    //        = Number_of_point_of_dead_man_in_cell;

    //меняем номер (мертвой точки в ячейке соответствующей этой самой точке)
    //указывающий на элемент главного массива

    this->CellMap
        .Array_of_cells[addtional_variable][number_of_type_of_death_particle]
                       [Number_of_point_of_dead_man_in_cell] =
    ...
    float X_coord_last_point =
        this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
            .SpacePointArray[Number_of_points - 1]
            .Xcoordinate;

    // Array_of_points[Number_of_points-1]. Xcoordinate;
    float Y_coord_last_point =
        this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
            .SpacePointArray[Number_of_points - 1]
            .Ycoordinate;
    // Array_of_points[Number_of_points-1]. Ycoordinate;
    int Cell_num_last_point =
        this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
            .SpacePointArray[Number_of_points - 1]
            .number_of_point_in_cell;
    // Array_of_points[Number_of_points-1]. number_of_point_in_cell;
    //                long double Birth_time_last_point =
    //                this->ModelPoints.Vector_of_types[number_of_type_of_death_particle].
    //                        SpacePointArray[Number_of_points-1].BirthTime;

    // номер ячейки с точкой с крайним номером
    int addtional_variable_last_element =
        (int)floorf(10 * X_coord_last_point + matrix_scaling_factor * 5) +
        10 * matrix_scaling_factor *
            floorf(10 * Y_coord_last_point + matrix_scaling_factor * 5);

    this->CellMap
        .Array_of_cells[addtional_variable_last_element]
                       [number_of_type_of_death_particle][Cell_num_last_point] =
        number_of_dead_man;
    // Array_of_cells_a_minus[addtional_variable_last_element][Cell_num_last_point]
    // = number_of_dead_man;
    ...
    // Array_of_points[number_of_dead_man].Xcoordinate =
    // Array_of_points[Number_of_points-1].Xcoordinate;
    this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
        .SpacePointArray[number_of_dead_man]
        .Ycoordinate =
    ...
    // Array_of_points[number_of_dead_man].Ycoordinate =
    // Array_of_points[Number_of_points-1].Ycoordinate;
    this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
        .SpacePointArray[number_of_dead_man]
        .number_of_point_in_cell =
  ...
  else
  {
    ...
    // уменьшаем на 1 кол-во ячеек с размеров
    // Array_of_cells_a_minus[addtional_variable].size() так, как будет
    // уменьшено кол-во элементов в ячейке
    // Array_of_cells_a_minus[addtional_variable]
    ...
    // Array_numbers_of_sizes_of_cells[Array_of_cells_a_minus[addtional_variable].size()]
    // - 1;
    // если после удаления кол-во ячеек с размером
    // Array_of_cells_a_minus[addtional_variable].size() стало 0 и максимальное
    // кол-во элементов во всех ячейках совпадает с размером
    // Array_of_cells_a_minus[addtional_variable].size()
    ...
    // и увеличивам кол-во ячеек с размером
    // Array_of_cells_a_minus[addtional_variable].size()-1 на 1; проблем с -1
    // как индекса массива нет, так - как размер ячейки как минимум 1 ( одна в
    // будущем мертвая точка там имеется )
    ...
    // !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!//
    // ///////////////////////////////////////////////////////////////
    // если будущая мертвая точка посылаеться на поседний элемент
    // соответствующей ячейки то
    if(Cell_num_of_dad_man ==
       this->CellMap
               .Array_of_cells[addtional_variable]
                              [number_of_type_of_death_particle]
               .size() -
    ...
    {
      // если в будущем мертвая точка не посылаеться на поседний элемент ячеки
      // то записываем в M индекс главного массивая точек, на который ссылаеться
      // крайний элемент ячейки соответствующей в будущем мертвой точке

      int M = this->CellMap.Array_of_cells
                  [addtional_variable][number_of_type_of_death_particle]
                  [this->CellMap
                       .Array_of_cells[addtional_variable]
                                      [number_of_type_of_death_particle]
      ...
      // переписываем  посылание элемента (который соответствует в будущем
      // мертвой точке) ячейки (соотве. в будущем мертвой точке)
      ...
      this->ModelPoints.Vector_of_types[number_of_type_of_death_particle]
          .SpacePointArray[M]
          .number_of_point_in_cell = Cell_num_of_dad_man;
  ...
  //    for (int
  //    i=0;i<this->CellMap.Types_Array_numbers_of_sizes_of_cells[k].size()
  //    ;i++)
  //        cout <<"Types_Array_numbers_of_sizes_of_cells[k][i]="<<
  //               this->CellMap.Types_Array_numbers_of_sizes_of_cells[k][i]<<endl;
}

float simbad::gui::Model_of_space::rate_linear_on_configuration(
    int EventNumber, int Numberfunctionforcomponentrate,
    int Number_of_type_of_particle, int Number_of_point_in_type_array)
{
  //сумма всех
  float sum = 0.0;

  float Xcoord, Ycoord;
  Xcoord = this->ModelPoints.Vector_of_types[Number_of_type_of_particle]
               .SpacePointArray[Number_of_point_in_type_array]
               .Xcoordinate;
  Ycoord = this->ModelPoints.Vector_of_types[Number_of_type_of_particle]
               .SpacePointArray[Number_of_point_in_type_array]
               .Ycoordinate;
  int X_number = (int)floorf(10 * Xcoord + matrix_scaling_factor * 5);
  int Y_number = (int)floorf(10 * Ycoord + matrix_scaling_factor * 5);
  ...
  if(10 * Xcoord + matrix_scaling_factor * 5 - X_number - 0.5 <= 0)
  {
    X_indicator = -1;
  }
  else
  ...
  if(10 * Ycoord + matrix_scaling_factor * 5 - Y_number - 0.5 <= 0)
  {
    Y_indicator = -1;
  }
  else
  ...
  int addtional_variable_2 =
      addtional_variable + 10 * matrix_scaling_factor * Y_indicator;
  int addtional_variable_3 = addtional_variable +
                             10 * matrix_scaling_factor * Y_indicator +
                             X_indicator;
  ...
  for(int j = 0; (j < 4); j++)
  {
    for(int i = 0; i < this->CellMap
                           .Array_of_cells[Addtional_vailable[j]]
                                          [Numberfunctionforcomponentrate - 1]
                           .size();
        i++)
    {
      ...
      float difference_X =
          this->ModelPoints.Vector_of_types[Numberfunctionforcomponentrate - 1]
              .SpacePointArray[this->CellMap.Array_of_cells
                                   [Addtional_vailable[j]]
      ...
      {
        argument_of_function_a =
            pow(pow(difference_X, 2.0) + pow(difference_Y, 2.0), 0.5);
...
void simbad::gui::Model_of_space::set_conf_full_file_name(
    QString Conf_file_name)
{
  ConfFullFileName = Conf_file_name;
}

void simbad::gui::Model_of_space::set_result_full_fill_name(
...
void simbad::gui::Model_of_space::add_new_result_of_dynamics(
    result_of_event New_result)
{
  Vector_of_result_event.push_back(New_result);
}

void simbad::gui::Model_of_space::
    add_new_result_of_dynamics_one_beath_and_one_death(
        long double Time_of_event, float Xcoordinate_of_new_particle,
        float Ycoordinate_of_new_particle, float number_of_type_of_new_particle,
        float Xcoordinate_of_dead_particle, float Ycoordinate_of_dead_particle,
        float number_of_type_of_dead_particle)
{
  ...
  result_of_event A(Time_of_event, New_points_of_conf_after_event,
                    Dead_points_of_conf_after_event);
