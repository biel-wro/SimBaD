...
void GLWidjet_for_init_model::paintEvent(QPaintEvent *event)
{
  ...
  painter.drawText(
      260, 508,
      QString::number(-0.5 * Big_model->Proportion_for_radiuses_wrt_Biggest));
  painter.drawText(
      260, 12,
      QString::number(0.5 * Big_model->Proportion_for_radiuses_wrt_Biggest));
  ...
  painter.drawEllipse(
      CursorPoint.x() - X_Parent_Widjet_pos - BrushRadius * 0.5 - 7,
      CursorPoint.y() - Y_Parent_Widjet_pos - BrushRadius * 0.5 - 30,
...
void GLWidjet_for_init_model::drawSpacePoints(QPainter &painter)
{
  ...
  {
    painter.setPen(
        Big_model->ModelPoints.Vector_of_types[i].InitialConfigurationColor);

    for(int j = 0; j < Big_model->ModelPoints.Vector_of_types[i]
                           .Number_of_points_in_SpacePointArray;
        j++)
    {
      //+++
      //           painter.drawPoint(256+(int)
...
void GLWidjet_for_init_model::mousePressEvent(QMouseEvent *event)
{
  ...
  if(random_regime)
  {
    ...
    unsigned seedPoissonDistribution =
        std::chrono::system_clock::now().time_since_epoch().count() + 5;

    std::poisson_distribution<int> PoissonDistribution(
        (int)(IntensityValue * 4.0 / 3.1415926535897932384626433832795));
    // std::uniform_real_distribution <float>
    // uniform_real_distributionX(0,BrushRadius);
    std::uniform_real_distribution<float> uniform_real_distribution_coord(
        0, BrushRadius);
    ...
    {
      float X_probable_point =
          XCoord_Widjet + uniform_real_distribution_coord(mt) - BrushRadius / 2;
      float Y_probable_point =
          YCoord_Widjet + uniform_real_distribution_coord(mt) - BrushRadius / 2;
      if((X_probable_point <= 512) and (X_probable_point >= 0) and
         (Y_probable_point <= 512) and (Y_probable_point >= 0) and
         (((X_probable_point - XCoord_Widjet) *
               (X_probable_point - XCoord_Widjet) +
      ...
      )
      {
        ...
        //+++
        //            CurrentPoint.setXcoordinate((float)(X_probable_point)*1.0/512.0
        //            - 0.5);
        //            CurrentPoint.setYcoordinate((float)(Y_probable_point)*1.0/512.0
        //            - 0.5);
        //+++
  ...
  else
  {
    int k;
    for(int i = 1; i < 25; i++)
      if(((i * i) <= IntensityValue) && (((i + 1) * (i + 1)) > IntensityValue))
        k = i;
    k = int(k / 2.0) + 1;
    for(int i = (-k + 1); i < k; i++)
      for(int m = (-k + 1); m < k; m++)
      {
        float X_probable_point =
            XCoord_Widjet + i * (int(BrushRadius / (k * 1.0)) + 1);
        float Y_probable_point =
            YCoord_Widjet + m * (int(BrushRadius / (k * 1.0)) + 1);
  ...
  //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  if(j - 1 >= 0)
    Big_model->ModelPoints.Vector_of_types[Current_type]
