...
simbad::gui::Dialog_for_init_model::Dialog_for_init_model(
    bool new_configuration, QWidget *parent)
    : QDialog(parent), ui(new Ui::Dialog_for_init_model)
{
  ui->setupUi(this);
  new_config = new_configuration;
  if(new_config)
    this->ui->pushButton->setText("Save new config ...");
  else
    this->ui->pushButton->setText("Save configuration");
    ...
    on_pushButton_color_for_init_conf_clicked()
{
  ...
  Color = Color_Dialog.getColor(
      Qt::white, 0,
      "Choose the color of the particles for the initial configuration", 0);
...
void simbad::gui::Dialog_for_init_model::
    on_pushButton_color_for_dynamic_sim_clicked()
{
  QColorDialog Color_Dialog;
  QColor Color;
  Color = Color_Dialog.getColor(
      Qt::white, 0, "Choose the color of the particles for dynamic simulation",
      0);
...
void simbad::gui::Dialog_for_init_model::on_horizontalSlider_valueChanged(
    int value)
{
...
void simbad::gui::Dialog_for_init_model::setModel_of_space(
    Model_of_space *Model)
{
  Big_model = Model;
...
void simbad::gui::Dialog_for_init_model::
    on_comboBox_Type_of_particle_currentIndexChanged(int index)
{
  this->ui->widget->setCurrent_type(index);
  this->ui->widget->setCurrent_color_for_init_conf(
      this->Big_model->ModelPoints
          .Vector_of_types[this->ui->comboBox_Type_of_particle->currentIndex()]
          .InitialConfigurationColor);
  this->ui->widget->setCurrent_color_for_dynam_conf(
      this->Big_model->ModelPoints
          .Vector_of_types[this->ui->comboBox_Type_of_particle->currentIndex()]
          .DynamicSimulationColor);
  QPalette Pal_color_for_init_conf(palette());
  // set black background
  Pal_color_for_init_conf.setColor(
      QPalette::Background,
      this->Big_model->ModelPoints
          .Vector_of_types[this->ui->comboBox_Type_of_particle->currentIndex()]
          .InitialConfigurationColor);
  ...
  QPalette Pal_color_for_dynamic_sim(palette());
  // set black background
  Pal_color_for_dynamic_sim.setColor(
      QPalette::Background,
      this->Big_model->ModelPoints
          .Vector_of_types[this->ui->comboBox_Type_of_particle->currentIndex()]
          .DynamicSimulationColor);
...
void simbad::gui::Dialog_for_init_model::on_horizontalSlider_2_valueChanged(
    int value)
{
  ...
  if(new_config)
  {
    ...
    File_name =
        QFileDialog::getSaveFileName(this, tr("Save configuration file "),
                                     "new_configuration", tr("Model (*.cnf)"));
    if(File_name != "")
    {
...
void simbad::gui::Dialog_for_init_model::on_checkBox_Random_measure_clicked(
    bool checked)
{
