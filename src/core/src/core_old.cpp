...
Core::Core() noexcept : running(STOPPED), pCurrentModel(nullptr)
{
...
void Core::selectModel(const std::string &modelName) throw(std::out_of_range,
                                                           BusyException)
{
  {
    ...
    {
      throw BusyException(
          "can't change model while curent model is not stopped");
    }
  }
