...
char const *test_input =
    R"("position_0","position_1","position_2","float_property", "int_property","event.kind", "unused"
...
BOOST_AUTO_TEST_CASE(read_stream)
{
  ...
  description.standardize_record(ATTRIBUTE_KIND::EVENT_KIND);

  assert(5 == description.size());
  // std::cerr << description;
  // std::cerr.flush();

  std::vector<std::string> values_to_track = {
      "float_property", "int_property",
  };
