...
BOOST_AUTO_TEST_CASE(number_check)
{
  ...
  BOOST_REQUIRE_EQUAL(configuration.size(), nparticles);

  std::size_t visited = 0;
  configuration.visit_records(
...
BOOST_AUTO_TEST_CASE(single_check_3d)
{
  ...
  std::size_t idx = configuration.descriptor()
...
BOOST_AUTO_TEST_CASE(grid_check)
{
  ...
  std::size_t nparticles = configuration.size();
